...
public class play_sound_right_lv2 extends Activity {
    MediaPlayer mpBgm;
    Timer timer;
    Button btnOK;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.play_sound_2);//<---------พูกเข้ากับ  play_sound_2.xml
        ...
        btnOK.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                Intent a = new Intent(play_sound_right_lv2.this, insert_text_right_lv2.class);
                startActivity(a);
                timer.cancel();
            }
        }); // OnClick
    ...
    public class MyTask extends TimerTask {
        ...
        @Override
        public void run() {
            play_sound_right_lv2.this.runOnUiThread(new Runnable() {// <-------- รัน Runnable ในเธรดหลัก เพื่อจะได้เข้าถึง UI ได้
                @Override
                public void run() {

                    if (x == 0) {
                        mpBgm = MediaPlayer.create(play_sound_right_lv2.this, sound_right[currentNumber++]);
                        mpBgm.start();
...
//**************************************************************************************************************************
                        SharedPreferences sp = getSharedPreferences("Data_compare_sound_right_lv2", Context.MODE_PRIVATE);//Pref_name = Sound A = ชื่ออะไรก็ได้ เพราะมันจะไปกลายเป็นชื่อไฟล์ที่เก็บลงในเครื่อง
                        ...
                        editor.putInt("Data_compare_sound_right_lv2", currentNumber);
                        editor.commit();
